...
const MESSAGESCONTAINER = document.querySelector("#messages_wrapper");
...
async function setLobbyData() {
  const data = await Promise.all(
    Object.keys(LOBBY.players).map(async (playerRole) => {
      ...
      if (playerRole == "Tank") backgroundURL = "img/TANK.jpg";
      else if (playerRole == "Marksman") backgroundURL = "img/MARKSMAN.jpg";
      else if (playerRole == "Assassin") backgroundURL = "img/ASSASSIN.jpg";
      else if (playerRole == "Mage") backgroundURL = "img/MAGE.jpg";
      else if (playerRole == "Fighter") backgroundURL = "img/FIGHTER.jpg";
      else if (playerRole == "Support") backgroundURL = "img/SUPPORT.jpg";
    ...
    <script src="https://kit.fontawesome.com/d75cefc660.js" crossorigin="anonymous"></script>
      <link rel="stylesheet" type="text/css" href="css/lobby.css">
          <div style="background: url('${backgroundURL}')" class="player-card" >
            <h6>
              <a href="http://localhost/web/profile/view.php?user_id=${
                user.user_id
              }" class="link-light text-decoration-none" >${user.username}</a>
              <span class="badge bg-primary">${playerRole}</span>
            </h6>
            <div>
              <span>IGN: <span class="badge bg-success">${
                mlbbdata.ign
              }</span></span><br>
              <span>MLID: <span class="badge bg-danger">${
                mlbbdata.mlid
              }</span></span>
            </div>
            <div>
            ...
            <form method="POST" action="${
              friendship
                ? "http://localhost/server/api/friends/delete.php"
                : "http://localhost/server/api/friends/index.php"
            }">
            ${
              friendship
          ...
          ${
            ...
            `
              : `
            <span class="text-center"> Friend Request Sent</span>
